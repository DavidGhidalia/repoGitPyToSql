-- FUNCTION: public.get_results_extra(json)

DROP FUNCTION IF EXISTS public.get_results_extra(json);

CREATE OR REPLACE FUNCTION public.get_results_extra(
	results json)
...
SELECT r1.type, r1.field_name, r1.typekey, ot.id AS type_id
		FROM (
			SELECT r.type, r.extra_data ->> 'type'::text AS field_name,
					CASE 
			...
			FROM (
				...
				json_array_elements(CASE WHEN (markets.markets -> 'extraData')::text <> 'null' THEN markets.markets -> 'extraData'::text END) as extra_data
				FROM (
